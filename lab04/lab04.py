...
class ArrayList:
    ...
    def insert(self, idx, value):
        ...
        if len(self.data) == self.len:
          inputData = ConstrainedList(len(self.data) * 2 if len(self.data) != 0 else 1)
          for i in range(self.len + 1):
            if i < idx:
              inputData[i] = self.data[i]
            elif i == idx:
              inputData[i] = value
            else:
              inputData[i] = self.data[i - 1]
          self.data = inputData
        else:
    ...
    def index(self, value, i=0, j=None):
        ...
        for k in range(i, self._normalize_idx(j) if j and self._normalize_idx(j) < self.len else self.len):
    ...
    def __add__(self, other):
        ...
        of other."""
        ### BEGIN SOLUTION
        inputData = ArrayList(self.len + len(other))
        for i in range(self.len):
          inputData[i] = self.data[i]
        for j in range(len(other)):
          inputData[j + self.len] = other[j]
        return inputData
        ### END SOLUTION
    ...
    def extend(self, other):
        ...
        if (self.len + len(list(other)) > len(self.data)):
          inputData = ConstrainedList(self.len + len(list(other)) * 2)
          for i in range(self.len):
            inputData[i] = self.data[i]
          for i in range(len(list(other))):
            inputData[i + self.len] = list(other)[i]
          self.len = self.len + len(list(other))
          self.data = inputData
